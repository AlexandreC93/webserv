...
std::string handleGetRequest(std::vector<ServerBlock> &serverBlocks, const std::string &uri, int port) {
    ...
    // Trouver le bon serveur basé sur le port
    ServerBlock *serverBlock = nullptr;
    for (size_t i = 0; i < serverBlocks.size(); ++i) {
        if (serverBlocks[i].listen == port) {
            serverBlock = &serverBlocks[i];
            break;
        }
    }

    if (serverBlock == nullptr) {
    ...
    if (filepath == serverBlock->root + "/") {
        ...
        if (serverBlock->error_page.find(error_code) != serverBlock->error_page.end()) {
            ...
            std::cout << "Error page found for code " << error_code << ": " << filepath << std::endl;
        } else {
            filepath = "www/html/404.html"; // chemin par défaut si la page d'erreur n'est pas trouvée
            std::cout << "No error page defined for code " << error_code << ", using default: " << filepath << std::endl;
        }
    }
